...
public class LessonController {
    ...
    public ResponseEntity<?> getAllLessons(@PathVariable UUID moduleId, SpecificationTemplate.LessonSpec spec,
           @PageableDefault(page = 0, size = 10, sort = "lessonId", direction = Sort.Direction.ASC) Pageable pageable){
        return ResponseEntity
                .status(HttpStatus.OK)
                .body(this.lessonService.findAllBySpecification(SpecificationTemplate.lessonModuleId(moduleId).and(spec), pageable));
